...
export function BarChartComponent(properties, children) {
	...
	const xScale = (index) => padding.left + index * (barWidth + barSpacing) + barSpacing / 2;
	const yScale = (value) => chartHeight - padding.bottom - ((value - minValue + padding_y) / (valueRange + 2 * padding_y)) * graphHeight;
	...
	for (let i = 0; i <= numTicks; i++) {
		const value = minValue - padding_y + (i / numTicks) * (valueRange + 2 * padding_y);
